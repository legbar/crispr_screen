...
source("C:\\Users\\yez07\\Desktop\\Project\\RDRU\\rpgm\\FXN_results_process_screen_I_II_combined_sub.R")
...
## screen I
inf11 = paste0(outpath,"FXN_ctrl_sgrna_nointron.gene_summary.txt")
inf12 = paste0(outpath,"FXN_ctrl_sgrna_nointron.sgrna_summary.txt")
 
inf13 = paste0(outpath,"FXN_ctrl_sgrna.count.txt")
#inf14 = paste0(outpath,"FXN_ctrl_sgrna.count_normalized.txt")

gene_dat_scI = read.delim(inf11,header=TRUE) #19292
sgrna_dat_scI = read.delim(inf12,header=TRUE) #77736

raw_count_scI = read.delim(inf13,header=TRUE) ## sum(raw_count_scI$BL) 88216257,sum(raw_count_scI$FXN) 73132444
#norm_count = read.delim(inf14,header=TRUE) ## FXN 

raw_count_scI$BL_LOG = ifelse(raw_count_scI$BL>0,log10(raw_count_scI$BL),0)
raw_count_scI$FXN_LOG = ifelse(raw_count_scI$FXN>0,log10(raw_count_scI$FXN),0)


## screen II
inf21 = paste0(outpath,"FXN_SCREENII_NEG_CTRL_nointron.gene_summary.txt") 
inf22 = paste0(outpath,"FXN_SCREENII_NEG_CTRL_nointron.sgrna_summary.txt")

inf23 = paste0(outpath,"FXN_SCREENII.count.txt")
#inf24 = paste0(outpath,"FXN_SCREENII.count_normalized.txt")

gene_dat_scII = read.delim(inf21,header=TRUE) #19292
sgrna_dat_scII = read.delim(inf22,header=TRUE) #77736
...
outg11 = paste0(outpath,"sgRNA_scI_nointron_counts_FXN_BL.jpeg")

jpeg(outg11,width = 600, height = 600, units = "px", pointsize = 12,quality = 100)
plot(0,0,xlim=c(rg_min,rg_max),ylim=c(rg_min,rg_max),type="n",xlab="BL",ylab="FXN+",main="Screen I")
points(raw_count$BL_LOG,raw_count$FXN_LOG,col="red")
...
jpeg(outg12,width = 600, height = 600, units = "px", pointsize = 12,quality = 100)
plot(0,0,xlim=c(rg_min,rg_max),ylim=c(rg_min,rg_max),type="n",xlab="BL",ylab="FXN+",main="Screen II")
points(raw_count$BL2_LOG,raw_count$FXN2_LOG,col="red")
abline(a=0,b=1,lty=2,col="gray")
dev.off()
